...
import 'package:expedientes_clinicos/presentation/patient_visit/visit_information/patient_visit_category/patient_visit_category_form.dart';
...
class DropdownSearchPatientVisitCategory extends StatefulWidget {
  const DropdownSearchPatientVisitCategory({super.key});

  @override
  State<DropdownSearchPatientVisitCategory> createState() =>
      _DropdownSearchPatientVisitCategoryState();
}

class _DropdownSearchPatientVisitCategoryState
    extends State<DropdownSearchPatientVisitCategory> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
        create: (context) => getIt<PatientVisitTypeCategoryWatcherBloc>()
          ..add(const CategoryWatcherEvent.watchAllStarted()),
        child: BlocConsumer<PatientVisitTypeCategoryWatcherBloc,
            CategoryWatcherState>(listener: (context, state) {
          state.map(
              initial: (value) {
                categoryList = [];
              },
          ...
          return DropDownSearchHead(
            ...
            onSearchWithKey: (key) {
              context
                  .read<PatientVisitTypeCategoryWatcherBloc>()
            ...
            listElements: categoryList
                ...
                .toList(),
            hintText: AppStrings.visitType,
            newFunction: () {
              context
                  .read<StateRendererBloc>()
                  .add(StateRendererEvent.popUpForm(
                      title: AppStrings.createVisitType,
                      bodyWidget: PatientVisitCategoryForm(
                        category: Category.empty(),
                        onCreated: (Category category) {
                          context.read<PatientVisitFormBloc>().add(
                              PatientVisitFormEvent.onVisitTypeChanged(
                                  category));
                          searchFieldController.text =
