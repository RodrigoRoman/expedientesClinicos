...
@LazySingleton(as: IMedicineCategoryRepository)
class MedicineCategoryRepository implements IMedicineCategoryRepository {
  final FirebaseFirestore _firestore;
  MedicineCategoryRepository(this._firestore);
  ...
  Future<Either<CategoryFailures, Unit>> create(Category category) async {
    try {
      ...
      final categoryReference = FirebaseStorage.instance
          .ref()
          .child('medicineCategories/${categoryDto.id + categoryDto.name}.jpg');
      ...
      final categories = _firestore.collection('medicineCategories');
  ...
  Future<Either<CategoryFailures, Unit>> createFake() async {
    try {
      final cats = _firestore.collection('medicineCategories');
  ...
  Stream<Either<CategoryFailures, KtList<Category>>> watchAll() async* {
    final category = _firestore.collection('medicineCategories');
    yield* category
        .snapshots()
        .map(
